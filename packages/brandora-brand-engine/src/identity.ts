...
import type { ColorSwatch, Positioning, Typography } from "@brandora/shared";
import type { BrandBrief } from "./interview.js";
import { AA_CONTRAST, contrastRatio, ensureContrast, hslToHex, meanHue, type Hsl } from "./color.js";
 ...
 */
export const TRAIT_HUE: Record<string, number> = {
  warm: 25,
  elegant: 285,
  bold: 355,
  natural: 95,
...
/**
 * Positioning decides depth, not hue. Luxury goes deep and quiet; mass market
 * goes bright and loud. This is the single strongest signal in the brief, which
 * is why it is a required question.
 */
interface Tone {
  saturation: number;
  lightness: number;
  /** How far the accent swings from the primary, in degrees. */
  accentSpread: number;
}

export const POSITIONING_TONE: Record<Positioning, Tone> = {
...
export interface IdentityOptions {
  /** Increment to get a different palette from the same brief (§16). */
  variation?: number;
}

export function derivePalette(brief: BrandBrief, options: IdentityOptions = {}): ColorSwatch[] {
  const variation = options.variation ?? 0;
  const hues = brief.personality.map((trait) => TRAIT_HUE[trait]).filter((h): h is number => h !== undefined);
  ...
  const primary: Hsl = { h: baseHue, s: tone.saturation, l: tone.lightness };
  const secondary: Hsl = { h: baseHue + 28, s: tone.saturation * 0.75, l: tone.lightness + 0.18 };
  const accent: Hsl = { h: baseHue + tone.accentSpread, s: tone.saturation * 0.9, l: 0.52 };

  // Surface and ink carry a trace of the brand hue so pages feel of a piece
  // rather than "the brand colour, on grey".
  const surface: Hsl = { h: baseHue, s: 0.06, l: 0.97 };
...
/**
 * Type pairings, chosen for what they signal and for what actually renders on a
 * cheap Android phone over a slow connection. Every family here is available as
 * a variable web font with a Latin-Extended subset, which matters for French.
 */
interface Pairing {
  primary: string;
  secondary: string;
...
const PAIRINGS: Record<string, Pairing> = {
  serifDisplay: {
    ...
    rationale:
      "A high-contrast serif for the name and headings reads as considered and expensive; a plain sans keeps the small print legible.",
  },
  editorialSerif: {
  ...
  workhorseSans: {
    ...
    secondaryFallback: "system-ui, -apple-system, sans-serif",
    rationale:
      "One family, two weights. Cheapest to load, impossible to get wrong, and clear on a small screen.",
  },
};

export function deriveTypography(brief: BrandBrief): Typography {
  const has = (trait: string): boolean => brief.personality.includes(trait);

  const key =
    brief.positioning === "luxury"
      ? "serifDisplay"
      : has("elegant")
        ? "editorialSerif"
  ...
  if (!pairing) throw new Error("typography pairing table is empty");
  return {
 ...
 */
export function buildLogoBrief(
  brief: BrandBrief,
...
export function paletteIsAccessible(palette: readonly ColorSwatch[]): boolean {
  ...
  if (!surface || !ink || !primary) return false;
  return (
    contrastRatio(ink, surface) >= AA_CONTRAST && contrastRatio(primary, surface) >= AA_CONTRAST
  );
}
