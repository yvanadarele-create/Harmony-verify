 ...
 */
export function ensureContrast(colour: Hsl, against: string, target = AA_CONTRAST): Hsl {
  const startHex = hslToHex(colour);
  if (contrastRatio(startHex, against) >= target) return colour;
  ...
  for (let step = 1; step <= 100; step += 1) {
    ...
    if (contrastRatio(hslToHex(candidate), against) >= target) return candidate;
  }
  return colour;
}
...
export function meanHue(hues: readonly number[]): number {
  ...
  if (Math.abs(x) < 1e-9 && Math.abs(y) < 1e-9) return hues[0] ?? 0;
