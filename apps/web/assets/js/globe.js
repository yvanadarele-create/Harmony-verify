...
(function () {
  ...
  /* One instance per [data-globe] element. The hero carries a small one and the
     experts page a large one, and a module that assumed a single node meant the
     second globe silently did nothing — the kind of bug that looks like a CSS
     problem for an hour. */
  function createGlobe(stage) {
    var canvas = document.createElement("canvas");
    stage.appendChild(canvas);
    var ctx = canvas.getContext("2d");
    if (!ctx) return;

    /* A 200px globe cannot carry 620 dots or ten arcs — they collapse into
       noise. Density scales with the rendered size rather than being fixed. */
    var mini = stage.dataset.globe === "mini";
    var field = mini ? FIELD.filter(function (_, i) { return i % 3 === 0; }) : FIELD;
    var routes = mini ? ROUTES.slice(0, 4) : ROUTES;
    var hubs = mini ? HUB_VECTORS.filter(function (_, i) { return i % 2 === 0; }) : HUB_VECTORS;
  ...
  document.querySelectorAll("[data-globe]").forEach(createGlobe);

  // Screen readers get the same information as a list, not a canvas they cannot see.
  document.querySelectorAll("[data-globe-list]").forEach(function (summary) {
    summary.textContent =
